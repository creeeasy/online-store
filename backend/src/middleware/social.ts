...
const socialInquiryTest = async (req: Request, res: Response, next: NextFunction) => {
  try {
    console.log(
      "------------------------------------------------------------------------------------------"
    );
    console.log("Starting socialInquiryTest middleware");
    ...
    if ((matchedReferrer || matchedUserAgent || matchedRequestedWith) && botScore === 0) {
      console.log("Social media traffic detected - proceeding to createInquiry controller");
      return next();
    }

    // For non-social traffic or bot traffic, create fake order
    console.log("Non-social media traffic or bot detected - creating fake order");
    ...
    console.error("Error in socialInquiryTest middleware:", {
...
export default socialInquiryTest;